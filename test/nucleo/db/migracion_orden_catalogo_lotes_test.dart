// Prueba de la migración de esquema v7 -> v8 (TE-20: `OrdenCatalogo` sigue
// el contrato real de `GET /api/sync/catalogo` — `lotes[]` en vez de
// `lote_id` único, HU-92 de `agrocom-api`; `litros_ha`/`kilos_por_vuelo`
// mutuamente excluyentes y nullable, HU-79 de `agrocom-api`).
//
// A diferencia de las migraciones anteriores (que preservan filas reales),
// acá la fila sembrada en v7 tiene el esquema VIEJO de `orden_catalogo`
// (`litros_ha NOT NULL`, sin `lote_id` en un arreglo) — incompatible con el
// contrato real de hoy. Es seguro perderla: `orden_catalogo` es un espejo de
// solo lectura de catálogo, nunca datos capturados sin conectividad (lo que
// protege la invariante "no perder filas" en las migraciones de
// `trabajo_local`/`sesion_local`), y el próximo pull la repuebla entera.
// Sigue el mismo patrón que `migracion_cierre_trabajo_test.dart`: arma a
// mano, con SQL crudo, el esquema v7 completo tal como quedó en dispositivos
// reales, fija `PRAGMA user_version = 7` y recién ahí abre [AppDatabase] con
// el `schemaVersion` real — drift dispara `onUpgrade(m, 7, 8)` de verdad.
...
/// SQL equivalente al esquema v7 real (ver `migracion_cierre_trabajo_test.dart`
/// para el detalle de cada tabla): `orden_catalogo` acá es DELIBERADAMENTE
/// el esquema VIEJO (`lote_id` único, `litros_ha NOT NULL`, sin
/// `kilos_por_vuelo`) que esta migración recrea.
const _createEsquemaV7 = '''
...
void main() {
  group(
    'migración de esquema drift (TE-20: contrato real de OrdenCatalogo)',
    () {
      test(
        'base v7 preexistente con catálogo y cursor del esquema viejo migra a '
        'v8: pierde el catálogo viejo a propósito, pero no toca cola_sync ni '
        'trabajo_local, y la tabla queda usable con el contrato nuevo',
        () async {
          // Arma a mano el estado "v7 preexistente" sobre un sqlite3.Database
          // crudo, antes de que drift lo toque.
          final rawDb = sqlite3.sqlite3.openInMemory();
          rawDb.execute(_createEsquemaV7);
          rawDb.execute('''
          INSERT INTO cola_sync
            (uuid_cliente, tipo_entidad, payload, secuencia, estado, creado_en)
          VALUES
            ('preexistente-v7', 'trabajo', '{"orden_id":1}', 1, 'confirmado', 1700000000);
        ''');
          rawDb.execute('''
        ...
        ''');
          ...
          // drift compara el user_version (7) recién fijado contra
          // schemaVersion y dispara onUpgrade(m, 7, 8) de verdad.
          final db = AppDatabase(NativeDatabase.opened(rawDb));
          addTearDown(db.close);

          // El catálogo viejo se pierde a propósito (ver comentario de
          // cabecera) — y el cursor se resetea con él, para que el próximo
          ...
          expect(filasColaSync.single.uuidCliente, 'preexistente-v7');
          ...
          // litros_ha nullable, kilos_por_vuelo disponible.
          await db
          ...
          final ordenNueva = (await db.select(db.ordenCatalogo).get()).single;
          expect(ordenNueva.litrosHa, isNull);
          expect(ordenNueva.kilosPorVuelo, Decimal.parse('8.50'));
        },
      );
    },
  );
